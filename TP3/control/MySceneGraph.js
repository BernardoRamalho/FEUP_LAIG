...
class MySceneGraph {
    ...
    parseInitials(initialsNode) {
        ...
        var rootIndex = nodeNames.indexOf("scenes");
        ...
        var scenesID = this.reader.getString(rootNode, 'scenesID');
        if (scenesID == null)
            return "No root id defined for scene.";

        this.sceneIDs = scenesID.split(",");
    ...
    parseLeaf(leaf, amplification){
        ...
        switch (leafType){
            ...
            case 'cube':
                return this.createCube(leaf);
            case 'pyramid':
                return this.createPyramid(leaf);
            case 'cubeMap':
                return this.createCubeMap(leaf);
        }
     ...
     * @param {Information of the Cube to be created} cubeInfo 
     */
    createCube(cubeInfo){

        // Get length from the XML
        let length = this.reader.getFloat(cubeInfo, 'length');
        if(length == null){
            this.onXMLError("Couldn't get the length value. Returning null");
            return null;
        }

        return new MyUnitCubeQuad(this.scene, length); // Create and Return a UnitCubeQuad
    }

    /**
     * Auxiliary function to parseLeaf.
     * Creates and Returns a pyramid.
     * @param {Information of the Pyramid to be created} pyramidInfo 
     */
    createPyramid(pyramidInfo){
        // Get height from the XML
        let height = this.reader.getFloat(pyramidInfo, 'height');
        if(height == null){
            this.onXMLError("Couldn't get the height value. Returning null");
            return null;
        }

        // Get sideLength from the XML
        let sideLength = this.reader.getFloat(pyramidInfo, 'sideLength');
        if(sideLength == null){
            this.onXMLError("Couldn't get the sideLength value. Returning null");
            return null;
        }

        // Get slices from the XML
        let slices = this.reader.getFloat(pyramidInfo, 'slices');
        if(slices == null){
            this.onXMLError("Couldn't get the slices value. Returning null");
            return null;
        }
        ...
        // Get stacks from the XML
        let stacks = this.reader.getFloat(pyramidInfo, 'stacks');
        if(stacks == null){
            this.onXMLError("Couldn't get the stacks value. Returning null");
            return null;
        }

        return new MyPyramid(this.scene, height, sideLength, slices, stacks); // Create and Return a Pyramid
    }

    /**
     * Auxiliary function to parseLeaf.
     * Creates and Returns a CubeMap.
     * @param {Information of the CubeMap to be created} cubeMapInfo 
     */
    createCubeMap(cubeMapInfo){
        // Get frontTextureID from the XML
        let frontTextureID = this.reader.getString(cubeMapInfo, 'frontTextureID');
        if(frontTextureID !== null && this.textures[frontTextureID] === null){
            this.onXMLError("Front texture doens't exist. Returning null");
            return null;
        }
        
        // Get backTextureID from the XML
        let backTextureID = this.reader.getString(cubeMapInfo, 'backTextureID');
        if(backTextureID !== null && this.textures[backTextureID] === null){
            this.onXMLError("Back texture doens't exist. Returning null");
            return null;
        }

        // Get leftTextureID from the XML
        let leftTextureID = this.reader.getString(cubeMapInfo, 'leftTextureID');
        if(leftTextureID !== null && this.textures[leftTextureID] === null){
            this.onXMLError("Left texture doens't exist. Returning null");
            return null;
        }

        // Get rightTextureID from the XML
        let rightTextureID = this.reader.getString(cubeMapInfo, 'rightTextureID');
        if(rightTextureID !== null && this.textures[rightTextureID] === null){
            this.onXMLError("Right texture doens't exist. Returning null");
            return null;
        }

        // Get topTextureID from the XML
        let topTextureID = this.reader.getString(cubeMapInfo, 'topTextureID');
        if(topTextureID !== null && this.textures[topTextureID] === null){
            this.onXMLError("Top texture doens't exist. Returning null");
            return null;
        }

        // Get bottomTextureID from the XML
        let bottomTextureID = this.reader.getString(cubeMapInfo, 'bottomTextureID');
        if(bottomTextureID !== null && this.textures[bottomTextureID] === null){
            this.onXMLError("Bottom texture doens't exist. Returning null");
            return null;
        }

        // Get scale from the XML
        let scale = this.reader.getFloat(cubeMapInfo, 'scale');
        if(scale == null){
            this.onXMLError("Couldn't get the scale value. Returning null");
            return null;
        }

        return new MyCubeMap(this.scene, this.textures[frontTextureID], this.textures[backTextureID], this.textures[leftTextureID], this.textures[rightTextureID], this.textures[topTextureID], this.textures[bottomTextureID], scale); // Create and Return a CubeMap
    }
  ...
  parseNodes(nodesNode) {
     ...
     */
    displayScene() {
        let currentSceneID = this.sceneIDs[this.activeScene];
        this.drawNode(this.MyNodes[currentSceneID], "clear", null);
    }
    ...
    drawNode(node, parentTexture, parentMaterial){
        ...
        for( let x = 0 ; x < node.nodeDescendants.length ; x++ ){
            let descendantNode = this.MyNodes[node.nodeDescendants[x]];
